    ...
    def _handle_concentration_mode(self, buttons):
        if self.menu_button_pressed(buttons):
            ...
            if self.colorimeter.concentration is not None:
                self.colorimeter.screen_manager.show_message(f"Concentration saved: {self.colorimeter.concentration} mg/L", is_error=False)
            else:
                self.colorimeter.screen_manager.show_message(f"Concentration saved: None mg/L", is_error=False)
        elif self.blank_button_pressed(buttons):
